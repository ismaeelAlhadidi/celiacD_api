...
class EditStoreProfileRequest extends RequestForm {
    ...
    protected function validate() : bool {
        ...
        if( isset($this->request['StoreName']) ) {
        ...
        if( isset($this->request['StoreType']) ) {
        ...
        if( isset($this->request['WorkingHours']) ) {

            $this->request['WorkingHours'] = floatval($this->request['WorkingHours']);
            
            if(! filter_var($this->request['WorkingHours'] ,FILTER_VALIDATE_FLOAT)) return false;
        ...
        foreach($this->request as $key => $value) {
            ...
            if($key == 'UserId' || $key == 'store' || $key == 'user') continue;
